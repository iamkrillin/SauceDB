...
using DataAccess.Xamarin.Android.Linq;
...
namespace DataAccess.Xamarin.Android
{
    ...
    /// </summary>
    public class AndroidDataConnection : IDataConnection
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AndroidDataConnection"/> class.
        /// </summary>
        /// <param name="filename">The filename</param>
        public AndroidDataConnection(string filename)
        {
            _connectionString = string.Format("Data Source={0}; Version=3;", filename);
            _tConverter = new StandardCLRConverter();
            _commandGenerator = new AndroidCommandGenerator();
        ...
        {
            return new AndroidDataStore(filename);
        }
        ...
        {
            return new AndroidQueryProvider(dStore);
        }
        ...
        public IEnumerable<DBObject> GetSchemaTables(IDataStore dstore)
        {
            ...
            using (QueryData tables = dstore.ExecuteCommands.ExecuteCommandQuery(getTables, dstore.Connection))
            {
                foreach (QueryRow tab in tables)
                {
                    using (SqliteCommand getColumns = new SqliteCommand())
                    {
                        ...
                        using (QueryData columns = dstore.ExecuteCommands.ExecuteCommandQuery(getColumns, dstore.Connection))
                        {
                            ...
                            foreach (QueryRow col in columns)
                            {
                                t.Columns.Add(new Column()
                                {
                                    ...
                                    IsPrimaryKey = dstore.Connection.CLRConverter.ConvertToType<bool>(col.GetDataForRowField("pk")),
