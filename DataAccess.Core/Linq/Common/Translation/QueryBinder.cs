...
namespace DataAccess.Core.Linq.Common.Translation
{
    ...
    public class QueryBinder : DbExpressionVisitor
    {
        ...
        protected virtual Expression BindSelectMany(Type resultType, Expression source, LambdaExpression collectionSelector, LambdaExpression resultSelector)
        {
            ...
            if (mcs != null && mcs.Method.Name.Equals("DefaultIfEmpty", StringComparison.InvariantCultureIgnoreCase) && mcs.Arguments.Count == 1 && (mcs.Method.DeclaringType == typeof(Queryable) || mcs.Method.DeclaringType == typeof(Enumerable)))
            {
